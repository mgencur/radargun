...
public class OpenFilesMonitor extends JmxMonitor {
   ...
   public synchronized void run() {
      try {
         ...
         if (openFiles != null) {
            timeline.addEvent(Timeline.Category.sysCategory(OPEN_FILES), new Timeline.ValueEvent(openFiles));
         }
   ...
   public synchronized void start() {
      super.start();
      timeline.addEvent(Timeline.Category.sysCategory(OPEN_FILES), new Timeline.ValueEvent(0));
   }
